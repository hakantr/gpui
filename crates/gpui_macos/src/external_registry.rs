...
/// without a device and a queue (decision D-K16), and it is confined to this platform crate so that
/// reaching it takes a direct dependency on `gpui_macos` rather than on `gpui`.
///
/// What it carries:
///
...
/// full rebuild, which starts by acquiring a new producer from [`external_surface_producer`]. The
...
///
/// `ns_view` is the `NSView` pointer of a GPUI window — the very pointer `raw_window_handle` reports
/// for it, as the `ns_view` field of an `AppKitWindowHandle`. It is the macOS counterpart of the
/// `HWND` `gpui_windows::external_surface_producer` takes and of the `HtmlCanvasElement`
/// `gpui_web::external_surface_producer` takes. The result is `None` when the pointer is not a live
/// GPUI window's view, or when the window's own state is already locked, which on this platform is
/// the span of `PlatformWindow::draw` — so a producer is acquired outside the frame rather than from
/// inside GPUI's draw, and attempting it there yields `None` instead of a deadlock.
///
...
/// # Safety
///
/// This is the one place the three producer accessors do not read alike, and the difference is the
/// platform's rather than a style choice. `gpui_windows::external_surface_producer` is a safe
/// function because Win32 validates an `HWND` for it — `GetWindowLongPtrW` on a dead or foreign
/// handle simply returns zero — and `gpui_web::external_surface_producer` is safe because a
/// `HtmlCanvasElement` is a checked JS reference. AppKit offers no such validation for a bare view
/// pointer, so the obligation is stated here instead of pretended away:
///
/// `ns_view` must be a live `NSView`. The pointer a GPUI window reports through `HasWindowHandle`
/// is one, and so is any other valid view, which is simply refused. A dangling pointer, a freed
/// view, or a pointer to something that is not an Objective-C object at all is undefined behavior.
pub unsafe fn external_surface_producer(
    ns_view: NonNull<c_void>,
) -> Option<ExternalSurfaceProducer> {
    // SAFETY: forwarded to the caller's obligation above; the lookup itself checks that the view is
    // one of GPUI's own before it reads the window-state ivar out of it.
    let state = unsafe { crate::window_state_from_view(ns_view) }?;
    // `try_lock` rather than `lock`: a producer acquired from inside GPUI's own draw would otherwise
    // wait forever on the lock that draw already holds.
