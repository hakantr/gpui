...
- The local `../zed` checkout is a read-only source and comparison repository. Do not edit its
  files, create commits or branches in it, or otherwise change its Git state.
- If a requested library change is not present in Zed, either keep parity or use the deliberate
  divergence process below in this repository. Preparing or sending an upstream Zed change requires
...
- Outside the deliberate-divergence process below, repo-local changes are limited to standalone
  workspace wiring, dependency-path adaptations, the `gpui` package version, test-only
  adaptations, explicitly documented omissions of Zed-only integrations, provenance and
  extraction documentation, verification tooling, and lockfile updates required to build outside
  the Zed workspace. Those ordinary extraction changes must not alter GPUI runtime behavior,
  public API, or retained GPUI feature semantics.
...
a sync than a rewrite. Do not send or apply the change to Zed without separate explicit
authorization; a divergence remains local until upstream independently covers it or that authority
is granted.
